      ...
      { case CompK:
        ...
        case FuncDeclK:
          ...
          if(currentScope != globalScope) {
            fprintf(listing, "Error at line(%d), name=%s : Function Declaration only in global!!\m", t->lineno, funcName);
            break;
          }
          isForFunc = TRUE;
...
static void checkNode(TreeNode * t)
{
  ...
  { case ExpK:
      ...
      { case AssignK:
          ...
          else if(lhsType == IntegerArr && rhsType == Integer){
            fprintf(listing, "ERROR at line(%d) : Variable type does not match\n", t->lineno);
          }
          else if(lhsType == Integer && rhsType == IntegerArr){
            fprintf(listing, "ERROR at line(%d) : Variable type does not match\n", t->lineno);
          }
          else
              t->type = lhsType;
        ...
        case OpK:
          ...
          { TreeNode* left = t->child[0];
            ...
            if(left->type == IntegerArr && left->child[0] != NULL){
                // Element of Array
                leftType = Integer;
            }
            if(right->type == IntegerArr && right->child[0] != NULL){
                // Element of Array
                rightType = Integer;
            }
