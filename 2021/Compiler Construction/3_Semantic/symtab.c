...
void st_insert(ScopeList scope, char * name, ExpType type, int lineno, int location, int isFunc )
...
{   fprintf(listing,"Variable Name  Variable Type  Scope Name  Location   Line Numbers\n");
    ...
    while(scope != NULL){
        for(int i = 0; i < SIZE; i++){
            ...
            while(bucket != NULL){
                fprintf(listing, "-%14s", bucket->name);
                if(bucket->isFunc == 1)
                    fprintf(listing, "Function");
                else{
                    if(bucket->type == Integer)
                        fprintf(listing, "Integer");
                    else if(bucket->type == IntegerArr)
                        fprintf(listing, "Integer[]");
                    else
                        fprintf(listing, "Void");
                }
                fprintf(listing, "%5s%5d", scope->name, bucket->memloc);
