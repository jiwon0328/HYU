...
firstPos = np.array([0, 0, 0])
cowCount = 0
cowsPos = [0 for i in range(6)]
isNotInitialClick = -1
returnCow = np.identity(4)
timeStart = 0
...
def getHeadDir(p0, p1, p2, p3, t):
    ...
    xang = math.atan2(direction[1], np.sqrt(direction[0]**2 + direction[2]**2))
    yang = math.atan2(direction[2], direction[0])
    zang = 0
...
def display():
    global cameraIndex, cow2wld, timeStart, startTime, cowPos, returnCow, spline, cowCount, isDrag, isNotInitialClick
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAGi, cowCount, cowsPos, cow2wld, isNotInitialClick
    if cowCount == 6:
        return
